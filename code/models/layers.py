...
class ToeplitzLayer:
  ...
  def __init__(self, shape_in, shape_out, alpha,
    ...
    init = tf.convert_to_tensor(np.float32(circ))
...
class CirculantLayer:
  ...
  def __init__(self, shape_in, shape_out, kernel_initializer=None,
    ...
    if kernel_initializer is None:
      ...
      kernel_initializer[0] = 1 + np.random.normal(0, 0.01)
      kernel_initializer = np.float32(kernel_initializer)
      self.kernel_real = tf.get_variable(name='kernel_real',
        initializer=kernel_initializer, regularizer=regularizer)
      self.kernel_imag = tf.get_variable(name='kernel_imag',
        initializer=kernel_initializer, regularizer=regularizer)
    else:
      self.kernel_real = tf.get_variable(name='kernel_real', shape=shape,
        initializer=kernel_initializer, regularizer=regularizer)
      self.kernel_imag = tf.get_variable(name='kernel_imag', shape=shape,
        initializer=kernel_initializer, regularizer=regularizer)
    ...
    if use_diag:
      self.diag_real = tf.get_variable(name='diag_real', # shape=[shape_out],
        initializer=diag_initializer, regularizer=regularizer)
      self.diag_imag = tf.get_variable(name='diag_imag', # shape=[shape_out],
        initializer=diag_initializer, regularizer=regularizer)

    if use_bias:
      self.bias_real = tf.get_variable(name="bias_real", shape=[shape_out],
        initializer=bias_initializer, regularizer=regularizer)
      self.bias_imag = tf.get_variable(name="bias_imag", shape=[shape_out],
        initializer=bias_initializer, regularizer=regularizer)

  def matmul(self, input_data):
    input_data = tf.cast(input_data, tf.complex64)
    kernel = tf.dtypes.complex(self.kernel_real, self.kernel_imag)
    diag = tf.dtypes.complex(self.diag_real, self.diag_imag)
    bias = tf.dtypes.complex(self.bias_real, self.bias_imag)

    padding_size = (np.abs(self.size - self.shape_in))
    paddings = ((0, 0), (padding_size, 0))
    data = tf.pad(input_data, paddings) if self.padding else input_data
    act_fft = tf.spectral.fft(data)
    kernel_fft = tf.spectral.fft(kernel)
    ret_mul = tf.multiply(act_fft, kernel_fft)
    ret = tf.spectral.ifft(ret_mul)
    ret = ret[..., :self.shape_out]
    if self.use_diag:
  ...
  def __init__(self, shape_in, shape_out, start=False,
    ...
    if normal_init:
      ...
      if sign_init:
        ...
        diag_initializer = np.float32(np.random.choice([-1., 1.],
                                                       size=[shape_out]))
      ...
      if rand_init:
        ...
        noise = tf.random_uniform(tf.shape(self.kernel), minval=-0.01, maxval=0.01)
...
class LowRankLayer:
  ...
  def matmul(self, input_data):
    ...
    ret = tf.matmul(input_data, self.kernel1)
    ret = tf.matmul(ret, self.kernel2)
    if self.use_bias:
      ret = ret + self.bias
    return ret
