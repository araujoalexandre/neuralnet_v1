...
def build_graph(reader, model, loss_fn, batch_size, regularization_penalty):
    ...
    with tf.device(device_string.format(i)):
      with tf.variable_scope("tower", reuse=reuse):
          ...
          logits_img, logits_adv = model.create_model(
            tower_inputs[i], n_classes=reader.n_classes, is_training=True,
                    labels=tower_labels[i], loss_fn=loss_fn.calculate_loss)
          tower_logits_img.append(logits_img)

          loss_img = loss_fn.calculate_loss(
            logits=logits_adv, labels=tower_labels[i])
          loss_img = add_reg_to_loss(loss_img, regularization_penalty,
                          scope="tower/network_defense")

          if FLAGS.train_attack['learn_noise_attack']:
            loss_adv = -add_reg_to_loss(loss_img, regularization_penalty,
          ...
          var_list_img = tf.global_variables(scope='tower/network_defense')
          ...
          if FLAGS.train_attack['learn_noise_attack']:
            var_list_adv = tf.global_variables(scope='tower/network_attack')
  ...
  train_op_img = train_op_cls_img.make_update(full_gradients_img, global_step)
...
class Trainer(object):
  ...
  def recover_model(self, meta_filename):
    ...
    return tf.train.import_meta_graph(meta_filename,
      clear_devices=FLAGS.clear_devices, import_scope="tower/network_defense")

  def build_model(self, model, reader):
    """Find the model and build the graph."""

    loss_fn = find_class_by_name(FLAGS.loss, [losses, tf.nn])()
    ...
    # var_list = tf.global_variables(scope="tower/network_defense")
    # var_list += [tf.train.get_or_create_global_step()]
...
def main():
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    trainer = Trainer(cluster, task, FLAGS.train_dir, model,
                      reader, batch_size)
