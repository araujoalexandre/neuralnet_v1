...
class VariableMgr(object):
  ...
  def __init__(self, benchmark_cnn):
    ...
    self.use_resource_vars = FLAGS.use_resource_vars
...
class VariableMgrLocalReplicated(VariableMgr):
  ...
  def preprocess_device_grads(self, device_grads):
    compact_grads = (FLAGS.use_fp16 and FLAGS.compact_gradient_transfer)
    defer_grads = (FLAGS.variable_consistency == 'relaxed')

    grads_to_reduce = [[g for g, _ in grad_vars] for grad_vars in device_grads]
    algorithm = batch_allreduce.algorithm_from_params(FLAGS)
    reduced_grads, self._warmup_ops = algorithm.batch_all_reduce(
        grads_to_reduce, FLAGS.gradient_repacking,
        compact_grads, defer_grads, FLAGS.xla_compile)
