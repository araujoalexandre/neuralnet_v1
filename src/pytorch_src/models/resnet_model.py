...
class BasicBlock(nn.Module):
  ...
  def __init__(self, in_planes, planes, affine, stride=1):
...
class ResNet(nn.Module):
  ...
  def __init__(self, params, num_classes, is_training):
    ...
    self.layer1 = self._make_layer(block, 16, num_blocks[0], affine, stride=1)
    self.layer2 = self._make_layer(block, 32, num_blocks[1], affine, stride=2)
    self.layer3 = self._make_layer(block, 64, num_blocks[2], affine, stride=2)
  ...
  def _make_layer(self, block, planes, num_blocks, affine, stride):
    strides = [stride] + [1]*(num_blocks-1)
    layers = []
    for stride in strides:
      layers.append(block(self.in_planes, planes, affine, stride))
