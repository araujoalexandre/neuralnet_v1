...
class DeepSpeech2Model(model_lib.Model):
  ...
  def accuracy_function(self, inputs, logits):
    ...
    return {
        (UNREDUCED_ACCURACY_OP_PREFIX + self.PROBABILITY_TENSOR):
            probs,
        (UNREDUCED_ACCURACY_OP_PREFIX + self.LABEL_TENSOR):
            inputs[1],
    }
  ...
  def postprocess(self, results):
    ...
    logging.info('total CER: {:f}; total WER: {:f}; total example: {:d}.'.format(
