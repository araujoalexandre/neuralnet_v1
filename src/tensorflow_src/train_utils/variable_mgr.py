...
class VariableMgr(object):
  ...
  def __init__(self, trainer):
    ...
    self.use_resource_vars = self.trainer.params.use_resource_vars
...
class VariableMgrIndependent(VariableMgr):
  ...
  def preprocess_device_grads(self, device_grads):
    return (self.trainer.devices, device_grads)

  def get_gradients_to_apply(self, device_num, gradient_state):
    device_grads = gradient_state
    tower_grad = device_grads[device_num]

    if self.trainer.enable_auto_loss_scale and device_num == 0:
  ...
  def get_devices(self):
    return self.trainer.raw_devices
...
class VariableMgrLocalFetchFromPS(VariableMgr):
  ...
  def preprocess_device_grads(self, device_grads):
    return ([self.trainer.param_server_device], device_grads)
  ...
  def get_gradients_to_apply(self, device_num, gradient_state):
    ...
    agg_grads, self.grad_has_inf_nan = (
        ...
        aggregate_gradients_using_copy_with_variable_colocation(
            device_grads,
            use_mean=True,
            check_inf_nan=self.trainer.enable_auto_loss_scale))
    return agg_grads

  def get_devices(self):
    raw_devices = self.trainer.raw_devices
    if self.trainer.local_parameter_device_flag == 'gpu':
      return [
      ...
      return [
          tf_v1.train.replica_device_setter(
              worker_device=d,
              ps_device=self.trainer.param_server_device,
...
class VariableMgrLocalReplicated(VariableMgr):
  ...
  def __init__(self, trainer, all_reduce_spec,
               agg_small_grads_max_bytes, agg_small_grads_max_group,
               allreduce_merge_scope):
    super(VariableMgrLocalReplicated, self).__init__(trainer)
    if all_reduce_spec:
  ...
  def preprocess_device_grads(self, device_grads):
    compact_grads = (self.trainer.params.use_fp16
                     and self.trainer.params.compact_gradient_transfer)
    defer_grads = (self.trainer.params.variable_consistency == 'relaxed')

    grads_to_reduce = [[g for g, _ in grad_vars] for grad_vars in device_grads]
    algorithm = batch_allreduce.algorithm_from_params(self.trainer.params)
    reduced_grads, self._warmup_ops = algorithm.batch_all_reduce(
        grads_to_reduce, self.trainer.params.gradient_repacking,
        compact_grads, defer_grads, self.trainer.params.xla_compile)
    if self.trainer.enable_auto_loss_scale:
    ...
    return self.trainer.devices, reduced_device_grads
...
class VariableMgrDistributedAllReduce(VariableMgr):
  ...
  def __init__(self, trainer, all_reduce_spec, job_name,
               num_workers, agg_small_grads_max_bytes,
               agg_small_grads_max_group, allreduce_merge_scope):
    super(VariableMgrDistributedAllReduce, self).__init__(trainer)
...
class VariableMgrCollectiveAllReduce(VariableMgr):
  ...
  def __init__(self, trainer, all_reduce_spec,
    ...
    super(VariableMgrCollectiveAllReduce, self).__init__(trainer)
    if not all_reduce_spec:
      raise ValueError(
...
class VariableMgrDistributedFetchFromPS(VariableMgr):
  ...
  def create_outer_variable_scope(self, device_num):
    if self.trainer.local_parameter_device_flag == 'gpu':
      caching_devices = self.trainer.raw_devices
    else:
      caching_devices = [self.trainer.cpu_device]
    custom_getter = variable_mgr_util.OverrideCachingDevice(
        caching_devices, self.trainer.cpu_device, 1024 * 64)
  ...
  def get_devices(self):
    ps_strategy = tf.contrib.training.GreedyLoadBalancingStrategy(
        self.trainer.num_ps, tf.contrib.training.byte_size_load_fn)
    return [
        tf_v1.train.replica_device_setter(
            worker_device=d,
            cluster=self.trainer.cluster_manager.get_cluster_spec(),
            ps_strategy=ps_strategy) for d in self.trainer.raw_devices
    ]
  ...
  def append_apply_gradients_ops(self, gradient_state, opt, grads, training_ops,
    ...
    def get_apply_gradients_ops_func():
      ...
      for i, (g, v) in enumerate(grads):
        ...
        barrier = self.trainer.add_sync_queues_and_barrier(
            'replicate_variable_%s' % i, [apply_gradient_op])
        with tf.control_dependencies([barrier]):
          with tf.device(self.trainer.cpu_device):
            updated_value = v.read_value()
            for my_d in range(len(self.trainer.devices)):
  ...
  def get_post_init_ops(self):
    ...
    for v in tf.global_variables():
      if v.name.startswith(variable_mgr_util.PS_SHADOW_VAR_PREFIX + '/v0/'):
        ...
        for i in range(self.trainer.num_gpus):
          name = 'v%s%s' % (i, prefix)
